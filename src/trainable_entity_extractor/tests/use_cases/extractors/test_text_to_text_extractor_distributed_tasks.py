...
class TestTextToTextExtractorDistributedTasks(TestCase):
    def setUp(self):
    ...
    def test_get_distributed_tasks_includes_date_parser_methods(self):
        extraction_data = self.create_sample_extraction_data()
    ...
    def test_get_distributed_tasks_includes_ner_methods(self):
        extraction_data = self.create_sample_extraction_data()
    ...
    def test_get_distributed_tasks_includes_same_input_output_method(self):
        extraction_data = self.create_sample_extraction_data()
    ...
    def test_get_distributed_tasks_with_date_content(self):
        extraction_data = self.create_sample_extraction_data()
    ...
    def test_get_distributed_tasks_gpu_requirements_are_boolean(self):
        extraction_data = self.create_sample_extraction_data()
    ...
    def test_get_distributed_tasks_with_gliner_methods(self):
        extraction_data = self.create_sample_extraction_data()
        ...
        if len(gliner_methods) > 0:
            gliner_tasks = [task for task in tasks if "Gliner" in task.method_name]
            for task in gliner_tasks:
                self.assertTrue(task.gpu_needed, "Gliner methods should require GPU")

    def test_get_distributed_tasks_with_empty_label_text(self):
        extraction_data = self.create_sample_extraction_data()
        for sample in extraction_data.samples:
