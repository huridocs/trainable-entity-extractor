...
class TextBalancedSetFit2(TextToMultiOptionMethod):
    ...
    @staticmethod
    def add_no_value_option(data: list[tuple[str, list[int]]]):
        no_value_option_data = list()
        for i in range(len(data)):
            if sum(data[i][1]) == 0:
                no_value_option_data.append((data[i][0], data[i][1] + [1]))
            else:
                no_value_option_data.append((data[i][0], data[i][1] + [0]))

        return no_value_option_data

    def get_balanced_data(self, data: list[tuple[str, list[int]]]):
        balanced_data = list()
        rows_added = set()
        data = self.add_no_value_option(data)
    ...
    def add_rows_to_balanced_data(balanced_data, columns_counts, data, rows_added, max_samples):
        for row_index, (text, labels) in enumerate(data):
            ...
            if any(label and columns_counts[i] < max_samples for i, label in enumerate(labels)):
                ...
                balanced_data.append((text, labels[:-1]))
    ...
    def train(self, extraction_data: ExtractionData):
        ...
        args = TrainingArguments(
            ...
            max_steps=int(get_max_steps(len(extraction_data.samples)) * 2),
