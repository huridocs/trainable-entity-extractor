    ...
    def get_balanced_data(self, data: list[tuple[str, list[int]]]):
        ...
        for text, labels in data:
            for i, label in enumerate(labels):
                if not label:
                    continue
                if i not in columns_counts:
                    columns_counts[i] = 0
                columns_counts[i] += 1

        columns_under_limit = [k for k, v in columns_counts.items() if v <= self.MAX_SAMPLES]
        balanced_data = list()
        rows_included = list()
        for j, (text, labels) in enumerate(data):
            for i, label in enumerate(labels):
                if not label:
                    continue
                if i in columns_under_limit:
                    balanced_data.append((text, labels))
                    rows_included.append(j)
                    break

        columns_counts = dict()
        for text, labels in balanced_data:
            for i, label in enumerate(labels):
                if not label:
                    continue
                if i not in columns_counts:
