...
from trainable_entity_extractor.use_cases.extractors.segment_selector.methods.frequent_6_words.SegmentFrequent6Words import (
    SegmentFrequent6Words,
)
...
class Frequent6Words:
    def __init__(self):
        self.segments: list[SegmentFrequent6Words] = list()
        self.model = None
        self.best_cut = 0
    ...
    def set_segments(self, pdfs_segments: list[PdfData]):
        ...
        for pdf_features in pdfs_segments:
            self.segments.extend(SegmentFrequent6Words.from_pdf_features(pdf_features))
    ...
    def save_most_frequent_words(self, model_path):
        ...
        number_repeated_words = 6
        most_repeated_words = [x[0] for x in count.most_common()[:number_repeated_words]]
        most_repeated_words = most_repeated_words + [""] * (number_repeated_words - len(most_repeated_words))
