...
from trainable_entity_extractor.use_cases.extractors.segment_selector.methods.next_previous_title.SegmentNextPreviousTitle import (
...
class NextPreviousTitle:
    def __init__(self):
        self.segments: list[SegmentNextPreviousTitle] = list()
        self.model = None
        self.best_cut = 0
    ...
    def set_segments(self, pdfs_segments: list[PdfData]):
        ...
        for pdf_features in pdfs_segments:
            self.segments.extend(SegmentNextPreviousTitle.from_pdf_features(pdf_features))
