...
from trainable_entity_extractor.use_cases.extractors.segment_selector.methods.base_frequent_words.SegmentBaseFrequentWords import (
    SegmentBaseFrequentWords,
)
...
class BaseFrequentWords:
    def __init__(self):
        self.segments: list[SegmentBaseFrequentWords] = list()
        self.model = None
        self.best_cut = 0
    ...
    def set_segments(self, pdfs_segments: list[PdfData]):
        ...
        for pdf_features in pdfs_segments:
            self.segments.extend(SegmentBaseFrequentWords.from_pdf_features(pdf_features))
