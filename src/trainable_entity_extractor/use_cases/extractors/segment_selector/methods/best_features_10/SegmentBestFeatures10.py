...
class SegmentBestFeatures10:
    ...
    @staticmethod
    def get_other_segment_features(segment: "SegmentBestFeatures10"):
        if not segment:
            return list(np.zeros(22))
    ...
    @staticmethod
    def from_pdf_features(pdf_features: PdfData) -> list["SegmentBestFeatures10"]:
        modes = Modes(pdf_features)
        segments: list["SegmentBestFeatures10"] = list()
        for index, pdf_segment in enumerate(pdf_features.pdf_data_segments):
            segment_landmarks = SegmentBestFeatures10(index, pdf_segment, pdf_features, modes)
            segments.append(segment_landmarks)

        sorted_pdf_segments = sorted(segments, key=lambda x: (x.page_index, x.top))

        previous_title_segment: Optional[SegmentBestFeatures10] = None
