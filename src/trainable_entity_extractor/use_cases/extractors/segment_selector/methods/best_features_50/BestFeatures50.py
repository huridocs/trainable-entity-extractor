...
from trainable_entity_extractor.use_cases.extractors.segment_selector.methods.best_features_50.SegmentBestFeatures50 import (
    SegmentBestFeatures50,
)
...
class BestFeatures50:
    def __init__(self):
        self.segments: list[SegmentBestFeatures50] = list()
        self.model = None
        self.best_cut = 0
    ...
    def set_model(self, x_train, y_train):
        ...
        good_indexes = [i for i, importance in enumerate(light_gbm_model.feature_importance()) if importance > 50]
        self.get_best_features_50_indexes_path().write_text(json.dumps(good_indexes))
    ...
    def set_segments(self, pdfs_segments: list[PdfData]):
        ...
        for pdf_features in pdfs_segments:
            self.segments.extend(SegmentBestFeatures50.from_pdf_features(pdf_features))
    ...
    def predict(self, model, testing_pdfs_segments: list[PdfData], model_path):
        ...
        good_indexes = json.loads(self.get_best_features_50_indexes_path().read_text())
        x = x[:, good_indexes]
    ...
    def get_best_features_50_indexes_path(self) -> Path:
        return Path(str(self.model_path) + "_best_features_50_indexes")
