...
class OllamaRun(BaseModel):
    ...
    prompt: str = ""
    code: str = ""
    training_samples: list[OllamaSample] = list()
    non_used_samples: list[OllamaSample] = list()
    mistakes_samples: list[OllamaSample] = list()
    ...
    def _update_data_from_previous_run(self, previous_run: "OllamaRun" = None):
        if not previous_run:
            return
    ...
    def run_training(self, previous_run: "OllamaRun"):
        if not self.max_training_size or not OLLAMA_API_KEY:
            return
    ...
    def _process_samples_with_function(self, extract_func: callable, samples: list[OllamaSample]) -> list[str]:
        outputs_texts = []
    ...
    def run_code(self, samples: list[OllamaSample]) -> list[str] | list[list[str]]:
        if not self.code:
    ...
    @staticmethod
    def from_extractor_identifier(extraction_identifier: ExtractionIdentifier) -> "OllamaRun":
        path = Path(extraction_identifier.get_path()) / CODE_FILE_NAME
        code = path.read_text(encoding="utf-8") if path.exists() else ""
        return OllamaRun(code=code)

    @staticmethod
    def _get_empty_results(samples: list[OllamaSample]) -> list[str]:
        return [""] * len(samples)
