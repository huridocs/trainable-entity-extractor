...
class TestMergeParagraphs(TestCase):
    ...
    def test_merge_paragraphs_when_two_segments_corresponds_to_one(self):
        ...
        self.assertEqual("Text 1. en", segments_from_languages[0].segments[1].text_content)
        self.assertEqual("Text 1. tr Text 2. tr", segments_from_languages[1].segments[1].text_content)
    ...
    def test_merge_paragraphs_when_one_segments_corresponds_to_two(self):
        ...
        segmentation_issue = [segments[0], segments[1].merge(segments[2])]
        language_segment_1 = SegmentsFromLanguage(language="en", segments=segmentation_issue, is_main_language=True)

        language_segment_2 = SegmentsFromLanguage(language="tr", segments=self.get_segments("tr"), is_main_language=False)
        ...
        self.assertEqual(2, len(segments_from_languages[0].segments))
        self.assertEqual(2, len(segments_from_languages[1].segments))

        self.assertEqual("Text 0. en", segments_from_languages[0].segments[0].text_content)
        self.assertEqual("Text 0. tr", segments_from_languages[1].segments[0].text_content)

        self.assertEqual("Text 1. en Text 2. en", segments_from_languages[0].segments[1].text_content)
        self.assertEqual("Text 1. tr Text 2. tr", segments_from_languages[1].segments[1].text_content)
